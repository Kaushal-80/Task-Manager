...
export default function DeleteTopic() {
  ...
  useEffect(() => {
    if (!id) {
      ...
      axios.get('/api/topics?id=' + id).then(res => {
          ...
          <div className="relative p-6">
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                <h3 className="text-lg font-medium text-secondary-900">Delete <span text-green-700>{topicInfo?.title}</span></h3>
                <div className="mt-2 text-sm text-secondary-500">Are you sure you want to delete this Topic? This action cannot be undone.</div>
              </div>
            </div>
